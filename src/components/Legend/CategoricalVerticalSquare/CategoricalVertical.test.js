...
import LegendCategoricalCircle from './';
...
test('Legend does not crash', () => {
  const tree = shallow(<LegendCategoricalCircle values={values} />);
  expect(toJson(tree)).toMatchSnapshot();
});

test('Legend with an empty array', () => {
  expect(() => shallow(<LegendCategoricalCircle values={[]} />))
    .toThrowError('values is empty');
});

test('Legend with an change color', () => {
  const wrapper = mount(<LegendCategoricalCircle values={values} />);
