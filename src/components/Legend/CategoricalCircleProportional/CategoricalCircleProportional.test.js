...
import LegendCategoricalVertical from './';
...
test('Legend does not crash', () => {
  const tree = shallow(<LegendCategoricalVertical values={values} />);
  expect(toJson(tree)).toMatchSnapshot();
});

test('Legend with an empty array', () => {
  expect(() => shallow(<LegendCategoricalVertical values={[]} />))
    .toThrowError('values is empty');
});

test('Legend with an change color', () => {
  const wrapper = mount(<LegendCategoricalVertical values={values} />);
